...
<script type="text/javascript">
  ...
  <div>
    ...
    <td>用户名</td><td>密码</td><td>手机号</td><td>邮箱</td><td>操作</td></tr>
    ...
    <tr>
    	...
    	<td>${u.getUsername()}</td><td>${u.getPassword()}</td><td>${u.getCellphone()}</td><td>${u.getEmail()}</td>
    	<td style="text-align: center;"><a href="updateUser.jsp?un=${u.getUsername()}&pwd=${u.getPassword()}&cell=${u.getCellphone()}&email=${u.getEmail()}">修改</a>&nbsp;&nbsp;<a href="UserServlets?opr=del&username=${u.getUsername()}" class="del">删除</a></td></tr>
   </c:forEach>
   <tr style="text-align: center;"><td><input type="submit" value="删除" /></td>
    <td colspan="3"><a href="GoodsServlet?opr=list">商品信息管理</a></td><td colspan="2"><a href="admin.jsp">管理首页</a></td></tr>
     </table>
     </form>
